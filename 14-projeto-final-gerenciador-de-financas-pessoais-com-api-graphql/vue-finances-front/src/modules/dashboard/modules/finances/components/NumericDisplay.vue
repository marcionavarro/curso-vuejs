    ...
    <v-card-text>
      <v-layout row wrap justify-end>
        <v-flex v-for="btn in buttons" :key="btn" xs4 pa-1>
          <v-btn :color="color" class="headline" @click="change(btn, 'add')">{{ btn }}</v-btn>
        </v-flex>

        <v-flex xs4 pa-1>
          <v-btn icon @click="trashAll"><v-icon>delete</v-icon></v-btn>
        </v-flex>

        <v-flex xs4 pa-1>
          <v-btn icon @click="change"><v-icon>backspace</v-icon></v-btn>
        </v-flex>
      </v-layout>
    </v-card-text>
  </v-card>
